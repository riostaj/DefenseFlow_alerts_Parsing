...
Combines all report types into a single interactive menu:
  [1] DefensePro        — parse alert logs → attack session CSV
  [2] Kentik            — parse alert logs → attack cycle CSV
  [3] Arbor             — parse alert logs → attack cycle CSV
...
Folder layout (defaults):
  Kentik/Input/     — Kentik alert log files
  Kentik/Reports/   — Kentik CSV and HTML output
  Arbor/Input/      — Arbor alert log files
  Arbor/Reports/    — Arbor CSV and HTML output
...
Usage:
    ...
    python radware_reports.py --report arbor-weekly --start 2026-03-01 --end 2026-03-30
...
ARBOR_INPUT    = SCRIPT_DIR / "Arbor"  / "Input"
ARBOR_REPORTS  = SCRIPT_DIR / "Arbor"  / "Reports"
KENTIK_INPUT   = SCRIPT_DIR / "Kentik" / "Input"
KENTIK_REPORTS = SCRIPT_DIR / "Kentik" / "Reports"
DP_INPUT       = SCRIPT_DIR / "Input"
DP_REPORTS     = SCRIPT_DIR / "Reports"
...
def _collect_log_sources(path: Path) -> tuple:
    ...
    *path* may be a zip file or a directory containing zips / alert*.log files.
    Nested zip members are unpacked automatically.
    Rotated archives are processed oldest-first so end events are never seen
    before their matching starts.
    """
    zip_handles: list = []
    sources: list = []
...
def dp_parse_logs(log_sources: list) -> tuple:
    ...
    for name, opener in log_sources:
        with opener() as fh:
            for line in fh:
                m712 = re.search(
                    r"DFC00712.*security policy (\S+) for protected object (\S+)", line)
                if m712:
                    policy_map[m712.group(2)] = m712.group(1)
    ...
    return start_events, end_events, policy_map


def dp_build_sessions(start_events: list, gap: timedelta,
                       policy_map: "dict | None" = None) -> list:
    ...
    for _key, group_iter in groupby(
        ...
        def _close():
            ...
            sessions.append({
                ...
                "PeakBW_bps":      max_bw,
                "ProtectedName":   po if po in policy_map else "",
                "PolicyName":      policy_map.get(po, ""),
...
def run_defensepro(log_dir: Path, out_file: Path,
    ...
    try:
        start_events, end_events, policy_map = dp_parse_logs(log_sources)
    finally:
    ...
    if policy_map:
        print(f"    Policy map (DFC00712)  : {len(policy_map)} entr{'y' if len(policy_map) == 1 else 'ies'}")

    gap          = timedelta(minutes=gap_minutes)
    all_sessions = dp_build_sessions(start_events, gap, policy_map)
    ...
    fieldnames = [
        ...
        "PeakBW_human", "PeakBW_bps", "ProtectedName", "PolicyName", "SourceLogFile",
    ]
...
def kentik_parse_logs(log_sources: list) -> tuple:
    attacks: dict  = {}
    policy_map: dict = {}
    for name, opener in log_sources:
        with opener() as fh:
    ...
    return attacks, policy_map


def kentik_build_rows(attacks: dict, time_range: TimeRange,
                       policy_map: "dict | None" = None) -> list:
    policy_map = policy_map or {}
    rows = []
    for kid in sorted(attacks):
        ...
        up_pos          = a["up"]
        protected_names = [po for po in up_pos if po in policy_map]
        policy_names    = [policy_map[po] for po in up_pos if po in policy_map]
        ...
        rows.append({
            ...
            "Mitigation_DOWN":    " | ".join(a["down"]),
            "ProtectedName":      " | ".join(protected_names),
...
def run_kentik(log_dir: Path, out_file: Path, time_range: TimeRange) -> None:
    ...
    try:
        attacks, policy_map = kentik_parse_logs(log_sources)
    finally:
        for _zh in zip_handles:
            _zh.close()
    ...
    rows = kentik_build_rows(attacks, time_range, policy_map)
    if time_range.active:
        print(f"  Attacks within filter     : {len(rows)}")
    ...
    fieldnames = [
        ...
        "Peak_Bandwidth", "Peak_Bandwidth_bps", "Peak_PPS", "Peak_PPS_raw",
        "Attack_Start", "Attack_End", "Duration_min",
        "Mitigation_UP", "Mitigation_DOWN", "ProtectedName", "PolicyName",
        "Source_Log_File",
    ]
    with open(out_file, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
...
# ══════════════════════════════════════════════════════════════════
#  Weekly HTML report  (shared — parameterized by source)
# ══════════════════════════════════════════════════════════════════
...
def weekly_find_csvs(csv_dir: Path, source: str) -> list:
    """Find attack-cycle CSV files for the given source ('arbor' or 'kentik')."""
    files = sorted(csv_dir.glob(f"{source}_attack_cycles_*.csv"), reverse=True)
    if not files:
        print(f"  WARNING: No {source}_attack_cycles_*.csv found in {csv_dir}", file=sys.stderr)
    return files
...
def weekly_generate_html(weeks: dict, start_dt, end_dt, csv_files: list,
                          source: str) -> str:
    """Generate weekly HTML trend report.
    source: 'arbor' or 'kentik'
    """
    id_field     = "Arbor_ID"    if source == "arbor"  else "Kentik_ID"
    id_label     = "Arbor ID"    if source == "arbor"  else "Kentik ID"
    source_title = "Arbor"       if source == "arbor"  else "Kentik"
    extra_label  = "Policy Name" if source == "arbor"  else "Protected Object Name"
    extra_field  = "Policy_Name" if source == "arbor"  else "Mitigation_UP"
    ...
    def _attack_detail_json(r: dict) -> str:
        ...
        fields = [
            ...
            ("Duration",        dur_human),
            (extra_label,       r.get(extra_field, "") or "—"),
        ]
...
function showAttackDetail(type) {{
    ...
    const title  = type === 'bw' ? '\u26a1 Peak Bandwidth Attack Detail' : '\U0001F4E2 Peak PPS Attack Detail';
...
def run_weekly_report(time_range: TimeRange,
    ...
    print()
    print("┌" + _separator("─", 48) + "┐")
    print(f"│          {source_title} Weekly HTML Trend Report{' ' * (14 - len(source_title))}│")
    print("└" + _separator("─", 48) + "┘")
    ...
    csv_files = weekly_find_csvs(csv_dir, source)
    if not csv_files:
        print(f"  ERROR: No source CSV files found. Run the {source_title} parser first.")
        return

    print(f"  Found {len(csv_files)} CSV file(s)")

    attacks = weekly_load_attacks(csv_files, start_dt, end_dt, id_field)
    if not attacks:
    ...
    if out_path is None:
        ...
        out_path = csv_dir / f"{source}_weekly_report_{ts}.html"
    out_path.parent.mkdir(parents=True, exist_ok=True)

    html = weekly_generate_html(weeks, start_dt, end_dt, csv_files, source)
...
def main_menu() -> str:
    """Present the report selection menu.
    Returns one of: dp, kentik, arbor, kentik-weekly, arbor-weekly
    """
    print()
    print("╔" + _separator("═", 52) + "╗")
    ...
    print("    [3] Arbor             — parse logs → attack cycle CSV")
    print("    [4] Kentik Weekly HTML  — attack cycle trends (HTML report)")
    print("    [5] Arbor Weekly HTML   — attack cycle trends (HTML report)")
    print("    [Q] Quit")
    print()

    while True:
        choice = input("  Selection [1/2/3/4/5/Q]: ").strip().upper()
    ...
    return {"1": "dp", "2": "kentik", "3": "arbor", "4": "kentik-weekly", "5": "arbor-weekly"}[choice]
...
def parse_args():
    p = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            "  python radware_reports.py\n"
            "  python radware_reports.py --report kentik-weekly --start 2026-03-01 --end 2026-03-30\n"
            "  python radware_reports.py --report arbor-weekly\n"
            "  python radware_reports.py --report dp --interactive\n"
            "  python radware_reports.py --report arbor --config alert_parser.ini\n"
        ),
    )
    p.add_argument("--report",
                   choices=["dp", "kentik", "arbor", "kentik-weekly", "arbor-weekly"],
    ...
    p.add_argument("--start", help="Start date  YYYY-MM-DD or YYYY-MM-DD HH:MM:SS")
    p.add_argument("--end",   help="End date    YYYY-MM-DD or YYYY-MM-DD HH:MM:SS")
...
def main():
    ...
    def _resolve(cli_val: str, cfg_section: str, cfg_key: str, default: Path) -> Path:
        raw = cli_val or _cfg_str(cfg, cfg_section, cfg_key)
        if raw:
    ...
    #  Weekly HTML reports
    # ══════════════════════════════════════════════════════════════
    if report_choice in ("kentik-weekly", "arbor-weekly"):
        source   = "kentik" if report_choice == "kentik-weekly" else "arbor"
        default_csv_dir = KENTIK_REPORTS if source == "kentik" else ARBOR_REPORTS
        csv_dir  = Path(args.csv_dir).resolve() if args.csv_dir else default_csv_dir
        out_path = Path(args.out).resolve() if args.out else None
        run_weekly_report(time_range, csv_dir, out_path, source)

    # ══════════════════════════════════════════════════════════════
    #  Parser reports  (DefensePro / Kentik / Arbor)
    # ══════════════════════════════════════════════════════════════
    else:
        if report_choice == "dp":
            dp_log_dir = _resolve(args.log_dir, "defensepro", "log_dir", DP_INPUT)
            dp_out_str = (
                args.out
                or _cfg_str(cfg, "defensepro", "out_file")
        ...
        elif report_choice == "kentik":
            k_log_dir = _resolve(args.log_dir, "kentik", "log_dir", KENTIK_INPUT)
            k_out_str = (
                args.out
                or _cfg_str(cfg, "kentik", "out_file")
                or f"Kentik/Reports/kentik_attack_cycles_{now_str}.csv"
            )
        ...
        elif report_choice == "arbor":
            a_log_dir = _resolve(args.log_dir, "arbor", "log_dir", ARBOR_INPUT)
            a_out_str = (
                args.out
                or _cfg_str(cfg, "arbor", "out_file")
                or f"Arbor/Reports/arbor_attack_cycles_{now_str}.csv"
            )
            a_out_file = (SCRIPT_DIR / a_out_str
                          .replace("{datetime}", now_str)
                          .replace("{date}", today)).resolve()
            run_arbor(a_log_dir, a_out_file, time_range)
